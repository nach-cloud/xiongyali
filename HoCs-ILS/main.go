...
func main() {
	...
	name := "data/4.11data/HILS.csv"
	iterations := []int{1, 5, 10, 100, 1000}
				...
				for index, date := range dates {
					if date {
						...
						for i := 1; i <= core.DecideCount; i++ { //i代表第几次决策
							...
							//约简有权无向图
							s, _ := graph.HILS(core.NewSolution(graph), iterations[iteration])
							//s := graph.HILS(iteration)
