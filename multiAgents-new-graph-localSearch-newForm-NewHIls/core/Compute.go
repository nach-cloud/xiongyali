...
func CopyStringSlice(src []string) []string {
	...
	for i, v := range src {
		// 为每个字符串创建新的内存空间并复制内容
		copyStr := make([]byte, len(v))
		copy(copyStr, v)
		// 将复制的字符串转为 string 类型并赋值给新切片
		dst[i] = string(copyStr)
	}

	return dst
}

// generateMateKey generates a unique key for a mate pair
func generateMateKey(u, v string) string {
	if u < v {
		return u + "|" + v
	}
	return v + "|" + u
