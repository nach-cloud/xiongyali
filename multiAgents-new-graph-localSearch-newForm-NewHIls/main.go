			...
			for index, date := range dates {
				if date {
					...
					for i := 1; i <= core.DecideCount; i++ { //i代表第几次决策
						...
						OriginGraph := core.BuildGraph(taskPoints, drones, cars, workers, chargePoints, actions, i)
						graph := core.NewGraph()
						s := core.NewSolution(graph)
						isOver := true
						n := 1
						for len(OriginGraph.NodeQueues) > 0 && isOver {

							for key, queue := range OriginGraph.NodeQueues {
								k := 0
								for queue.Len() > 0 && k < n {
									k = k + 1
									heap.Init(OriginGraph.NodeQueues[key])
									node := heap.Pop(queue).(core.GraphNode)
							...
							for i, node1 := range nodes {
								for j, node2 := range nodes {
									if i != j && core.IsConflict(node1, node2) {
							...
							//约简有权无向图
							isChange := true
							s, isChange = graph.HILS(s)
							if !isChange {
								n = n * 2
							}
							isOver = false
							for key, _ := range OriginGraph.NodeQueues {
								flag := false
								for so := range s.GetSolution() {
									flag = graph.Nodes[so].Car != nil && graph.Nodes[so].Car.GetUUId() == key || graph.Nodes[so].Drone != nil && graph.Nodes[so].Drone.GetUUId() == key || graph.Nodes[so].Worker != nil && graph.Nodes[so].Worker.GetUUId() == key
									if flag {
										break
									}
								}
